...
const items = sourcePaths(source).map(({ data, fill }) => ({
  item: paper.PathItem.create(data),
...
const masked = items.map(({ item, fill }) => {
  const result = item.subtract(mask)
  return { data: result.pathData, fill }
})
...
const paths = masked
  .map(({ data, fill }) => `  <path fill="${xml(fill)}" d="${xml(data)}"/>`)
  .join('\n')
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="4 3 24 24">
  ...
  <rect x="4" y="3" width="24" height="24" fill="${BACKGROUND}"/>
${paths}
